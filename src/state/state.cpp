 ...
 * @return int
 */
int State::evaluate(bool max_player)
{
  int me;
    ...
    for (int j = 0; j < BOARD_W; j++)
    {
      ...
      if (now_piece)
      {
        ...
        case 1:
          if (this->player)
          {
            if (oppo_board[i + 1][j + 1] && i + 1 < BOARD_H && j + 1 < BOARD_W)
            {
              oppn_piece = oppo_board[i + 1][j + 1];

              h += chess_value[oppn_piece - 1];
            }
            if (oppo_board[i + 1][j - 1] && i + 1 < BOARD_H && j > 0)
            {
              oppn_piece = oppo_board[i + 1][j - 1];
          ...
          {
            if (oppo_board[i - 1][j + 1] && i > 0 && j + 1 < BOARD_W)
            {
              oppn_piece = oppo_board[i - 1][j + 1];

              h += chess_value[oppn_piece - 1];
            }
            if (oppo_board[i - 1][j - 1] && i > 0 && j > 0)
            {
              oppn_piece = oppo_board[i - 1][j - 1];
      ...
      else if (oppn_piece)
      {
        ...
        case 1:
          if (!this->player)
          {
            if (self_board[i + 1][j + 1] && i + 1 < BOARD_H && j + 1 < BOARD_W)
            {
              now_piece = self_board[i + 1][j + 1];

              h -= chess_value[now_piece - 1];
            }
            if (self_board[i + 1][j - 1] && i + 1 < BOARD_H && j > 0)
            {
              now_piece = self_board[i + 1][j - 1];
          ...
          {
            if (self_board[i - 1][j + 1] && i > 0 && j + 1 < BOARD_W)
            {
              now_piece = self_board[i - 1][j + 1];

              h -= chess_value[now_piece - 1];
            }
            if (self_board[i - 1][j - 1] && i > 0 && j > 0)
            {
              now_piece = self_board[i - 1][j - 1];
